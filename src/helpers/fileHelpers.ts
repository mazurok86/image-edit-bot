...
const allowedImageMimeTypes = ['image/jpeg', 'image/png', 'image/heic'] as const

const allowedVideoMimeTypes = ['video/quicktime', 'video/mp4'] as const
...
export function isAllowedVideo(mime: string, size: number): mime is AllowedVideoMime {
  return size < MAX_VIDEO_SIZE && includes(allowedVideoMimeTypes, mime)
}
