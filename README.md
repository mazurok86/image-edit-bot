...
Telegram-бот для редактирования изображений и генерации видео с помощью AI-моделей на [Replicate](https://replicate.com). Пользователь отправляет фото/видео/документ с текстовым запросом, выбирает модель — бот прогоняет вход через выбранную модель и возвращает результат.
...
- Настройка параметров под каждую модель (соотношение сторон, размер, разрешение, FPS, режим и т.д.) через inline-кнопки
...
- Персистентное состояние чатов в Redis; файлы истекают через час неактивности
...
1. Пользователь отправляет фото/видео/документ и текст запроса в Telegram.
2. `BotService` (`src/services/botService.ts`) обрабатывает все события Telegram и делегирует их обработчикам:
   ...
   - `FileHandler` — приём фото/видео/документов, конвертация HEIC → JPEG
...
Состояние каждого чата (`ChatState`) хранится в Redis через `ChatRegistry` / `ChatStore`; загруженные файлы истекают через час неактивности.

### Реестр моделей

Все модели описаны декларативно в `src/models/registry.ts`: Replicate ID, ограничения по числу изображений/видео, обязательность промпта и настраиваемые параметры (`capabilities`). Типобезопасность параметров обеспечивают типы в `src/types/model.ts` и `src/types/capabilities.ts`.

### Добавление новой модели

1. Добавить запись в `models` в `src/models/registry.ts` (id, ограничения, `requirePrompt`, `capabilities`).
2. Добавить метод в `ReplicateService`.
3. Добавить раннер в `runners` в конструкторе `BotService`.
